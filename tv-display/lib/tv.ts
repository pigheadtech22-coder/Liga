...
export async function fetchTvSnapshot(input: TvRequest = {}): Promise<TvSnapshot> {
  ...
  try {
    torneoRow = torneoId
      ? await pool.query(
          "SELECT id, nombre, descripcion, tv_theme, tv_header_logo_path, logo_left_path, logo_right_path, sponsor_logo_1_path, sponsor_logo_2_path, sponsor_logo_3_path, sponsor_logo_4_path, sponsor_logo_5_path, sponsor_logo_6_path, sponsor_logo_7_path, sponsor_logo_8_path FROM torneos WHERE id=$1 LIMIT 1",
          [torneoId]
        )
      : await pool.query(
          "SELECT id, nombre, descripcion, tv_theme, tv_header_logo_path, logo_left_path, logo_right_path, sponsor_logo_1_path, sponsor_logo_2_path, sponsor_logo_3_path, sponsor_logo_4_path, sponsor_logo_5_path, sponsor_logo_6_path, sponsor_logo_7_path, sponsor_logo_8_path FROM torneos ORDER BY id DESC LIMIT 1"
        );
  } catch (error) {
  ...
  const torneo = torneoRow.rows[0]
    ? {
        ...
        tv_theme: torneoRow.rows[0].tv_theme ? String(torneoRow.rows[0].tv_theme) : null,
