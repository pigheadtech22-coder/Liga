...
# ═══════════════════════════════════════════════════════════════
    ...
    if tv_readonly:
        st.markdown(
            ...
            div[data-testid="stVerticalBlockBorderWrapper"] p {font-size:0.82rem !important; margin:0 !important;}
            </style>
            """,
            unsafe_allow_html=True,
        )
            ...
            with cols[col]:
                ...
                with st.container(border=True):
                    ...
                    for j in jugadores_orden:
                        ...
                        with cf:
                            mostrar_foto(j.get("foto_sin_fondo", ""), j.get("foto_original", ""), size=46)
                        with cn:
                            st.markdown(f"<span style='font-size:0.82rem'>**P{j.get('posicion', '-')} · {j.get('nombre', '-')}**</span>", unsafe_allow_html=True)
